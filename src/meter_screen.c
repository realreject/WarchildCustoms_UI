...
void create_meter_screen()
{
    ...
    lv_obj_set_style_bg_color(meter_screen, lv_color_hex(0x050505), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_t *gauge_bg = lv_img_create(meter_screen);
    lv_img_set_src(gauge_bg, &temperature_gauge_320_320);
    lv_obj_align(gauge_bg, LV_ALIGN_CENTER, 0, 0);

    pointer_needle = lv_img_create(meter_screen);
    lv_img_set_src(pointer_needle, &needle_000);
    lv_obj_align(pointer_needle, LV_ALIGN_CENTER, 0, 0);

    create_title(meter_screen, "METER", 0, 135); // Create the title using the common function
    create_home_button(meter_screen);
